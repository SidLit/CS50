...
def index():
    ...
    portfolio = db.execute("SELECT symbol, SUM(shares) as number_of_shares, price, total FROM transactions WHERE user_id = :user_id GROUP BY symbol",
...
def buy():
    ...
    if request.method == "POST":
        ...
        # Query database to see how much cash the user has available
        available_cash = rows[0]["cash"]
        price = stock["price"]
        ...
        db.execute("INSERT INTO transactions (user_id, symbol, shares, price, total) VALUES(:user_id, :symbol, :shares, :price, :total)", user_id=session["user_id"],
                    symbol=request.form.get("symbol"), shares=shares, price=price, total=total_price)
...
def sell():
    ...
    if request.method == "POST":
        ...
        db.execute("INSERT INTO transactions (user_id, symbol, shares, price) VALUES(:user_id, :symbol, :shares, :price)", user_id=session["user_id"],
                    symbol=request.form.get("symbol"), shares=-shares, price=price)
...
def deposit():
    ...
    if request.method == "POST":
        ...
        db.execute("UPDATE users SET cash = cash + :money WHERE id = :user_id", user_id=session["user_id"], money=money)
